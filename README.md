...
云揽镜是一款 Agent 驱动的视频创作平台，把「一个想法」到「一条成片」的完整链路收进同一个工作区：

**灵感立项 → 剧本（分集 / 场次）→ 资产与参考图 → 分镜与镜头 → AI 生图 → AI 生视频 → 分集合成 → 成片输出**

它不只是在界面上接了几个 AI 模型，而是用一套接近影视工业的结构化流程把它们串起来：剧本有分集和场次，分镜有景别、运镜、角度和焦段，镜头有首帧尾帧和转场，成片按镜序自动拼接。创作者既可以一格一格手工打磨，也可以把重复劳动交给 Agent，在一段对话里跑完从剧本拆解到镜头生成的整条流水线。
...
| 模块 | 当前能力 |
| --- | --- |
| 项目与团队 | 管理创作项目、项目成员与协作角色，一个项目承载一部完整作品 |
| 剧本创作 | 按「分集 → 场次」组织剧本，维护剧情结构，并把项目上下文交给 Agent |
| 资产管理 | 维护角色、场景、道具等可复用资产，为跨镜头一致性提供参考图 |
| 分镜制作 | 从剧本整理分镜，逐镜头编辑画面、景别、运镜、台词、音效与生成结果 |
| 图片生成 | 按镜头描述与参考图生成画面，支持文生图、图生图以及首帧 / 尾帧控制 |
| 视频生成 | 由画面或首帧 / 尾帧生成镜头视频，并保留可复用的生成提示词 |
| 分集合成 | 将该集所有镜头视频按镜序拼接为成片，优先零转码，编码不一致时自动回退重编码 |
| 任务与进度 | 后台生成任务统一排队与跟踪，实时查看排队、处理中与完成状态 |
...
1. **立项** —— 创建作品项目，确定题材、风格基调与协作成员，后续所有内容都挂在这个项目下。
2. **写剧本** —— 按「分集 → 场次」两级结构组织剧本；可以手写，也可以让 Agent 基于你的创意生成和润色。
3. **建资产** —— 录入角色、场景、道具等资产，并生成或上传参考图。资产会被镜头反复引用，是角色和场景跨镜头保持一致的关键。
4. **出分镜** —— 由剧本整理出分镜（Agent 可一键从剧本拆解，也可手动逐条添加），形成「分集 → 场次 → 镜头」的完整结构。
5. **定镜头** —— 逐格细化：景别、镜头运动、机位角度、焦段、预估时长、画面描述、台词旁白、音效配乐与转场方式。
6. **生画面** —— 以镜头描述与参考图（含首帧 / 尾帧及其提示词）生成图片，不满意可单独重跑某一格。
7. **生视频** —— 由画面或首帧 / 尾帧生成镜头视频片段，生成提示词随镜头保存，便于复用和微调。
8. **合成片** —— 选择某一分集，系统按镜序取出所有镜头视频，用 FFmpeg 拼接成完整成片（优先零转码，编码不一致时自动回退重编码）。

每一步都可以人工接管，也可以交给 Agent；中途回退修改同样支持——改完的镜头重新生成、重新合成即可。

### 镜头可以精细到什么程度

| 维度 | 可定义的内容 |
| --- | --- |
| 画面 | 画面内容描述、画面期望（用于引导 AI 生图） |
| 镜头语言 | 景别（远景 / 全景 / 中景 / 近景 / 特写）、运动（推 / 拉 / 摇 / 移 / 跟 / 升 / 降）、机位角度（平视 / 俯视 / 仰视）、焦段、摄像机装备 |
| 时间 | 预估时长（秒）、转场方式（切 / 淡入 / 淡出 / 溶 / 划） |
| 声音 | 台词与旁白、音效、配乐建议、整体声音描述 |
| 生成控制 | 参考图、外部参考图、首帧图与提示词、尾帧图与提示词、AI 生成图、AI 生成视频及其提示词 |
| 资产关联 | 出场角色、所属场景、使用道具（直接引用资产库条目） |
| 协作 | 自动镜号与排序、备注、自定义扩展字段、AI 生成标记与草稿状态 |

### 为什么是云揽镜

- **一条流水线，而不是一堆工具**：剧本、分镜、镜头、生成、合成首尾相连，不需要在多个平台之间反复搬运素材。
- **镜头级可控**：不是"输入一句话、吐出一个视频"，每一格的景别、运镜、台词、转场都能单独改，也能单独重生成。
- **资产复用保证一致性**：角色、场景、道具做成资产后被各镜头引用，避免"同一个角色每张图长得不一样"。
- **Agent 接管重复劳动**：从剧本拆解分镜到批量生成镜头，都可以在对话中交给 Agent，推理过程与工具调用全程可见。
- **模型与存储可替换**：云端走云揽川聚合网关，也可接本地 ComfyUI 工作流；媒体可存本地磁盘或 S3 兼容对象存储。
