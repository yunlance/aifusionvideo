...
public class StoryboardService {
    ...
    public void deleteEpisode(Long id) {
        ...
        if (isDemoByStoryboardId(episode.getStoryboardId())) {
            throw new BusinessException("演示项目的分镜为系统内置参考数据，不可删除");
        }
    ...
    public void deleteScene(Long id) {
        ...
        if (isDemoByStoryboardId(scene.getStoryboardId())) {
            throw new BusinessException("演示项目的分镜为系统内置参考数据，不可删除");
        }
        itemMapper.delete(new LambdaQueryWrapper<StoryboardItem>()
    ...
    public void deleteItem(Long id) {
        StoryboardItem item = itemMapper.selectById(id);
        if (item == null) {
            throw new BusinessException("分镜条目不存在: " + id);
        }
        if (isDemoByStoryboardId(item.getStoryboardId())) {
            throw new BusinessException("演示项目的分镜为系统内置参考数据，不可删除");
        }
        itemMapper.deleteById(id);
    }

    private boolean isDemoByStoryboardId(Long storyboardId) {
        Storyboard storyboard = storyboardMapper.selectById(storyboardId);
        return storyboard != null && projectService.isDemoProject(storyboard.getProjectId());
    }
