...
- **⚠️ 核心 ID 释义（最重要）**：
  - **剧本集 ID** (`scriptEpisodeId`)：代表该剧本集的数据库自增主键。仅用于调用剧本相关工具（如 `get_script_episode` 和 `save_script_scene_items`）。
  - **剧本场次 ID** (`scriptSceneItemId`)：代表剧本中每个具体场次记录的自增 ID，由 `save_script_scene_items` 成功保存后返回。
