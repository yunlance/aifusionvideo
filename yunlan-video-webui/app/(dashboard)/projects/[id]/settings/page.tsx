...
export default function ProjectSettingsPage() {
  ...
  // 是否内置演示项目（系统保护，不可删除）
  const isDemoProject = String(savedProperties.demo ?? "") === "true";
  ...
  const handleDeleteProject = async () => {
    if (!project) return;
    const ok = await confirm({ title: "彻底删除项目", description: "确定要删除该项目吗？此操作将永久移除所有剧本、分镜、素材等数据，不可恢复。", variant: "destructive", confirmText: "彻底删除项目" }); if (!ok) return;
    setDeleting(true);
    try {
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      ...
      <motion.div variants={itemVariants} className={cn(
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className={cn("text-sm font-semibold", isDemoProject ? "text-amber-500" : "text-destructive")}>
            {isDemoProject ? "演示项目保护" : "危险操作"}
          </h3>
        </div>
        {isDemoProject ? (
          <p className="text-xs text-muted-foreground">
            演示项目为系统内置参考数据，不可删除，可任意修改以体验完整创作流程，不影响系统与其他用户。
          </p>
        ) : (
          <>
            <p className="text-xs text-muted-foreground mb-3">
              删除项目将永久移除所有剧本、分镜、素材等数据，此操作不可恢复。
            </p>
            <Button
              variant="destructive"
