...
export function EditItemAssetsDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("保存关联素材失败:", err);
      toastApiError(err, "保存关联素材失败");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative bg-card border border-border/30 rounded-2xl shadow-2xl w-[600px] max-w-[95vw] max-h-[85vh] flex flex-col overflow-hidden z-10">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/20 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold">关联镜头素材</h3>
              <p className="text-[10px] text-muted-foreground">
                设置分镜镜头 #{item.shotNumber || item.autoShotNumber || "?"} 的关联素材
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-5">
          ...
          <div className="space-y-3">
            ...
            {characterItems.length === 0 ? (
              <p className="text-[11px] text-muted-foreground/60 italic pl-0.5">暂无项目角色素材，请先在素材库中添加</p>
            ) : (
          ...
          <div className="space-y-3 pt-2.5 border-t border-border/10">
            ...
            {sceneItems.length === 0 ? (
              <p className="text-[11px] text-muted-foreground/60 italic pl-0.5">暂无项目场景素材，请先在素材库中添加</p>
            ) : (
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
                {sceneItems.map((item) => {
          ...
          <div className="space-y-3 pt-2.5 border-t border-border/10">
            ...
            {propItems.length === 0 ? (
              <p className="text-[11px] text-muted-foreground/60 italic pl-0.5">暂无项目道具素材，请先在素材库中添加</p>
            ) : (
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
                {propItems.map((item) => {
