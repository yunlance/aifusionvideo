...
function SceneAssetPanel({
  ...
  // 直接从分镜 items 聚合素材项 ID（characterIds / sceneAssetItemId / propIds）
  // 批量查素材项详情，附带素材名称做辅助展示
  const loadAssets = useCallback(async () => {
    ...
    try {
      ...
      console.error("加载场次素材失败:", err);
      toastApiError(err, "加载场次素材失败");
    } finally {
      setLoading(false);
    }
  }, [sceneGroup]);
  ...
  const handleBatchGenConfirm = (selectedItems: SelectedAssetItem[]) => {
    ...
    // 触发 Agent Pipeline
    addPipeline({
      label: `批量生图 (${selectedItems.length} 个素材项)`,
      projectId,
      request: {
  ...
  const loadLinkedAssets = useCallback(async () => {
    ...
    try {
      ...
      const items = await Promise.all(
        allItemIds.map((id) => assetApi.getItem(id).catch((error) => {
          toastApiError(error, "加载镜头关联素材项失败");
          return null;
        }))
      );
      ...
      console.error("加载镜头关联素材失败:", err);
      toastApiError(err, "加载镜头关联素材失败");
    } finally {
...
function LinkedAssetGroup({
  ...
  // 将素材与其关联的素材项打平，展示为独立的项，和表格页完全一致
