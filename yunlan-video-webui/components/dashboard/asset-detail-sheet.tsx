    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[calc(100vh-2rem)] sm:max-h-[80vh] flex flex-col overflow-hidden bg-card/95 backdrop-blur-md rounded-2xl p-0 gap-0">
        <DialogHeader className="px-6 py-5 border-b border-border/30 shrink-0">
          <DialogTitle className="text-base font-semibold">从素材库选择封面</DialogTitle>
          <DialogDescription className="text-xs">点击展开项目中的素材，点击图片以选取该图片为当前素材封面</DialogDescription>
        </DialogHeader>
              ...
              <div key={a.id} className="border border-border/30 rounded-xl overflow-hidden bg-card/40 transition-all hover:bg-card/60">
                ...
                {isExpanded && (
                  <div className="p-4 border-t border-border/30 bg-black/10">
                    {!a.items?.length ? (
                      <div className="py-6 text-center text-xs text-muted-foreground/40">该素材暂无素材项</div>
                    ) : (
                             ...
                             >
                               {urlStr ? (
                                 ...
                                 <div className="w-full h-full flex flex-col items-center justify-center p-2 text-center text-muted-foreground/60 p-2">
                                   ...
                                   <span className="text-[10px] break-all line-clamp-2 leading-tight">{item.name || "无名素材"}</span>
                                 </div>
                               )}
                               {urlStr && (
...
function AssetItemEditPanel({
  ...
  const handleDelete = async () => {
    const ok = await confirm({ title: "删除素材项", description: "确定删除该素材项吗？此操作无法撤销。", variant: "destructive", confirmText: "确定删除" }); if (!ok) return;
    try {
      await assetApi.deleteItem(item.id);
      onDeleted();
    } catch (err) {
      console.error("删除素材项失败:", err);
      toast.error(getApiErrorMessage(err));
    }
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/30 bg-card/60 backdrop-blur-sm shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold truncate">{item.name || "未命名素材项"}</h3>
          <span className="text-[10px] text-muted-foreground/50">素材项编辑</span>
        </div>
        <button
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border/30 bg-card/60 backdrop-blur-sm shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold truncate">新建素材项</h3>
        </div>
        <button
...
export default function AssetDetailPanel(props: Props) {
  ...
  const handleDelete = async () => {
    ...
    const ok = await confirm({ title: "删除素材", description: "确定要删除该素材及其所有素材项吗？此操作无法撤销。", variant: "destructive", confirmText: "确定删除" }); if (!ok) return;
    try {
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border/20 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-semibold">新建素材</h3>
          </div>
          <button
            onClick={handleCreate}
  ...
  return (
    <div className="h-full relative overflow-hidden">
      ...
      {/* ======= 右侧：素材项编辑/创建面板（absolute 定位，从右侧滑入） ======= */}
      <div
        className={cn(
